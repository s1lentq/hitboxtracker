...
int CStudioModelRenderer::StudioDrawModel(int flags)
{
	...
	if (gHUD.m_iFOV < 90)
	{
		if (strstr(m_pCurrentEntity->model->name, "v_awp")
			|| strstr(m_pCurrentEntity->model->name, "v_scout")
			|| strstr(m_pCurrentEntity->model->name, "v_g3sg1")
			|| strstr(m_pCurrentEntity->model->name, "v_sg550"))
		{
			return 0;
		}
	}
	...
	if (flags & STUDIO_RENDER)
	{
		...
		if ((strstr(m_pCurrentEntity->model->name, "hostage") || strstr(m_pCurrentEntity->model->name, "scientist")) && cl_shadows->value)
		{
