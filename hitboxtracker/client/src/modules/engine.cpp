...
bool CEngine::LoadSecureClient_Init()
{
	...
	SetHook(pos, &m_LoadSecureClient);
...
bool CEngine::LoadInSecureClient_Init()
{
	...
	char patch[6];
	patch[0] = '\xBE';
	memcpy(&patch[1], &refproc, 4);
	patch[5] = '\x90';

	return memcpy(pos, &patch, sizeof(patch));
}
...
svc_func_t *CEngine::FindSVCFuncs()
{
	...
	memcpy(&gSVCfuncs, pSVC, sizeof(gSVCfuncs));
	return pSVC;
}
...
cl_enginefunc_t *CEngine::FindEngineFuncs()
{
	...
	memcpy(&gEngfuncs, pEngine, sizeof(gEngfuncs));
	return pEngine;
}
...
cldll_func_t *CEngine::FindClientFuncs()
{
	...
	memcpy(&gClientfuncs, pClient, sizeof(gClientfuncs));
	return pClient;
}
