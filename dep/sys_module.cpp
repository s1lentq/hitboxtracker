...
bool CSysModule::SetHook(byteptr_t addr, hook_t *hook)
{
	...
	// copy original bytes
	Q_memcpy(hook->m_originalBytes, addr, sizeof(hook->m_originalBytes));

	// make patch
	return patch(addr, &hook->m_jmpBytes, sizeof(hook->m_jmpBytes));
}
...
bool CSysModule::set(hook_t *hook)
{
	...
	return patch(hook->m_addr, &hook->m_jmpBytes, hook->m_size);
}
...
bool CSysModule::unset(hook_t *hook)
{
	...
	return patch(hook->m_addr, hook->m_originalBytes, hook->m_size);
}

bool CSysModule::patch(void *dst, void *src, size_t size)
{
	...
	{
		Q_memcpy(dst, src, size);
		ret = !!VirtualProtect(dst, size, OldProtection, &NewProtection);
	}
